...
| 003 | `003_Tokocrypto_Signed_API_Forbidden_2015.md` | Every signed Tokocrypto `/api/v3` call returns `-2015`. IP allowlist (`118.99.94.221`, confirmed) and key permissions (read+trade, confirmed) both ruled out → the key is not valid for the Binance-standard `.site` host. Tokocrypto balance sync and order placement are unavailable. | high | **open — needs operator action** |
