...
| 003 | `003_Tokocrypto_Signed_API_Forbidden_2015.md` | Every signed Tokocrypto `/api/v3` call returns `-2015` (invalid key / IP / permission). Tokocrypto balance sync and order placement are unavailable; the live order path is still completely unexercised. | high | **open — needs operator action** |
