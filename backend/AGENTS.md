...
- `seeder/` — `python -m tarakdingdung.composition.seeder`. Idempotent
  (create-if-missing, keyed by name): RBAC from `permission/role/user.json`,
  plus `strategy.json` — a conservative long-only momentum baseline on Indodax
  IDR majors, seeded **PAPER + enabled** so the engine can run a full cycle
  with no funds at risk. Promotion to LIVE is a deliberate `PATCH` on the
  strategy row, never a seed.
