"""Live order execution for Tokocrypto over the legacy signed `/open/v1` API.

Tokocrypto runs two signed API systems and this account's key is only valid on
the legacy one (`www.tokocrypto.com` `/open/v1/*`) — the Binance-standard
`/api/v3` host rejects it with `-2015`. Market data still comes from `/api/v3`
(unauthenticated, and the only place candles are actually served); only the
signed surface — this executor and the account source — uses `/open/v1`.
See docs/todos/003.
"""
